...
import { Wrench, Plus, X, Receipt } from "lucide-react";
...
export default function WorksPage() {
  ...
  const form = useForm<CreateWorkInput>({
    ...
    defaultValues: { status: "PLANNED", isTaxDeductible: false },
  });
  ...
  const createMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setShowForm(false);
      form.reset({ status: "PLANNED", isTaxDeductible: false });
    },
  });
  ...
  const taxDeductibleCost = (works || []).filter((w) => w.isTaxDeductible).reduce((sum, w) => sum + (w.cost || 0), 0);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        >
          ...
          {showForm ? t("works.cancel") : t("works.addWork")}
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
          <p className="text-sm text-slate-500 flex items-center gap-1"><Receipt size={14} /> {t("works.taxDeductible")}</p>
        ...
        >
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input {...form.register("title")} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">{t("works.property")}</label>
              <select {...form.register("propertyId")} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input {...form.register("contractor")} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">{t("works.cost")}</label>
              <input {...form.register("cost", { valueAsNumber: true })} type="number" step="0.01" className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" />
            </div>
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1">{t("works.status")}</label>
              <select {...form.register("status")} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                {workStatuses.map((s) => <option key={s} value={s}>{statusLabels[s] || s}</option>)}
              </select>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <input {...form.register("isTaxDeductible")} type="checkbox" id="taxDeductible" className="rounded" />
            <label htmlFor="taxDeductible" className="text-sm text-slate-700">{t("works.taxDeductibleLabel")}</label>
          </div>
          <div>
            <label className="block text-sm font-medium text-slate-700 mb-1">{t("works.description")}</label>
            <textarea {...form.register("description")} rows={2} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm" />
          </div>
          <button type="submit" disabled={createMutation.isPending} className="px-6 py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium">
            {createMutation.isPending ? t("works.creating") : t("works.createWork")}
          </button>
        </form>
      )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 border-b border-slate-200">
              <tr>
                ...
                <th className="text-right px-6 py-3 font-medium text-slate-500">{t("works.actions")}</th>
              </tr>
            </thead>
            <tbody>
              {works.map((work) => (
                <tr key={work.id} className="border-b border-slate-100 hover:bg-slate-50">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${statusColors[work.status] || "bg-slate-100"}`}>
                      {statusLabels[work.status] || work.status}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-right font-medium">{work.cost ? formatCurrency(work.cost) : "—"}</td>
                  <td className="px-6 py-4 text-center">{work.isTaxDeductible ? <Receipt size={14} className="mx-auto text-emerald-500" /> : "—"}</td>
                  <td className="px-6 py-4 text-right">
                    <button onClick={() => deleteMutation.mutate(work.id)} className="text-slate-400 hover:text-red-500">
                      <X size={16} />
                    </button>
                  </td>
                </tr>
